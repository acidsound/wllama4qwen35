...
A direct `.onnx` URL is normalized to its parent model repository because chat inference also requires the repository's tokenizer, config, and companion graph files. Do not enter the `.onnx_data` URL separately; Transformers.js downloads external data files according to `transformers.js_config.use_external_data_format` in `config.json`. If `tokenizer_config.json` does not embed `chat_template`, the app loads `chat_template.jinja` from the same model repository, removes unsupported `{% generation %}` markers for Transformers.js compatibility, and assigns the result to the tokenizer before formatting messages. Arbitrary standalone `.onnx` files and non-Hugging-Face URLs are not supported.
